...
public class Map_Generator_Spiral : MonoBehaviour
{
    [SerializeField] private GameObject _castle;

    private LastPlotData _lastPlotData;
    public static Map_Generator_Spiral Instance;
    ...
    public Direction.NESW NextPlotDir(Vector3 plotPos, Direction.NESW last_NextPlotDir)
    {
        ...
        if (_counter < _sameDirMax)
        {
            _counter++;

            nextPlotDir = last_NextPlotDir;
        }
        else 
        {
            if (_secondPass)
            ...
            _counter = 0;
            nextPlotDir = Direction.Next_Clockwise_Dir(last_NextPlotDir);
        }
